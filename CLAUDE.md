...
- HTTP client (`fetch`/`fetchWith`): compiler-special-cased forms that reuse route declarations for type safety; `compile_fetch` extracts the constructor's route entry metadata (method, path pattern, body/query field descriptors, response schema) at compile time and passes them as string constants alongside the compiled record payload to `knot_http_fetch_io`, which returns an IO thunk; the thunk builds the URL (substituting path params), serializes body fields to JSON, sets headers, executes the HTTP request via `ureq`, parses the JSON response, and wraps it in a `Result` ADT (`Ok {value: parsed}` or `Err {error: {status, message}}`); type inference (`try_infer_fetch`) fully handles these forms — it skips the constructor's `respond` field when type-checking the record argument and resolves the response type from `fetch_response_types` (populated from route declarations during `pre_register`)
