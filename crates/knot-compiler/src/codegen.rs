...
pub struct Codegen<M: cranelift_module::Module = ObjectModule> {
    ...
    /// `(<>field : T) =>` fold-dict callsite resolutions: application span → the
    /// synthetic spans of every collected `field` fragment. Codegen merges them
    /// with `base.unify` from `{}` and splices the result as the leading arg.
...
impl<M: cranelift_module::Module> Codegen<M> {
    ...
    fn declare_runtime_fns(&mut self) {
        ...
        // Maybe monad
        self.declare_rt("knot_maybe_bind", &[p, p, p], &[p]);
        ...
        self.declare_rt("knot_drop", &[p, p], &[p]);
    ...
    fn collect_declarations(&mut self, program: &ast::Expr) {
        ...
        // Register built-in Maybe impls for Functor, Applicative, Monad, Alternative
        self.register_builtin_maybe_impls();
    ...
    fn register_builtin_relation_impls(&mut self) {
        ...
        let impls = [
            ("Functor_Relation_map", 2),
            ("Applicative_Relation_yield", 1),
            ("Applicative_Relation_ap", 2),
            ("Monad_Relation_bind", 2),
            ("Alternative_Relation_empty", 0),
            ("Alternative_Relation_alt", 2),
            ("Foldable_Relation_fold", 3),
            ("Traversable_Relation_traverse", 2),
            ("Semigroup_Relation_append", 2),
            ("Sequence_Relation_take", 2),
            ("Sequence_Relation_drop", 2),
        ];
    ...
    // ── Built-in Maybe impls for HKT traits ──────────────────────

    /// Register mangled functions for Functor/Applicative/Monad/Alternative Maybe impls.
    fn register_builtin_maybe_impls(&mut self) {
        let impls = [
            ("Functor_Maybe_map", 2),
            ("Applicative_Maybe_yield", 1),
            ("Monad_Maybe_bind", 2),
    ...
    fn register_builtin_primitive_impls(&mut self) {
        ...
        let impls = [
            ...
            ("Num_Float_negate", 1),
            // Semigroup impls
            ("Semigroup_Text_append", 2),
            // Sequence impls
            ("Sequence_Text_take", 2),
            ("Sequence_Text_drop", 2),
        ];
        for (mangled, n_params) in &impls {
            // Don't register if already defined (by user code or the prelude)
            if self.user_fns.contains_key(*mangled) {
    ...
    /// Define Cranelift IR bodies for built-in Maybe impls of HKT traits.
    fn define_builtin_maybe_impls(&mut self) {
        ...
        // Functor_Maybe_map(db, f, m) → knot_maybe_map(db, f, m)
        define_if_registered!("Functor_Maybe_map", |cg: &mut Self, func_id: FuncId| {
            let mut sig = cg.module.make_signature();
            sig.params.push(AbiParam::new(cg.ptr_type)); // db
            sig.params.push(AbiParam::new(cg.ptr_type)); // f
            sig.params.push(AbiParam::new(cg.ptr_type)); // m
            sig.returns.push(AbiParam::new(cg.ptr_type));
            cg.build_function(func_id, sig, |cg, builder, entry| {
                let db = builder.block_params(entry)[0];
                let f = builder.block_params(entry)[1];
                let m = builder.block_params(entry)[2];
                let result = cg.call_rt(builder, "knot_maybe_map", &[db, f, m]);
                builder.ins().return_(&[result]);
            });
        });
        ...
        define_if_registered!("Monad_Maybe_bind", |cg: &mut Self, func_id: FuncId| {
            ...
            cg.build_function(func_id, sig, |cg, builder, entry| {
                ...
                let f = builder.block_params(entry)[1];
                let m = builder.block_params(entry)[2];
                let result = cg.call_rt(builder, "knot_maybe_bind", &[db, f, m]);
                builder.ins().return_(&[result]);
            });
        });
    ...
    fn define_functions(&mut self, program: &ast::Expr, _type_env: &TypeEnv) {
        ...
        // JSON: generic runtime encoder/decoder (trait machinery removed).
        self.define_stdlib_fn_1("toJson", "knot_json_encode");
        self.define_stdlib_fn_1("parseJson", "knot_json_decode_maybe");
        ...
        // Define built-in Maybe impls for HKT traits
        self.define_builtin_maybe_impls();
    ...
    /// Compile a `(<>field : T) =>` fold-dictionary: merge EVERY collected
    /// `field` fragment (registered as synthetic-span `ImplicitRef`s in
    /// `implicit_refs`, listed in `fold_dict_args[span]`) into one record via
    /// `base.unify` (`knot_record_unify`, right-biased), starting from `{}`.
    /// Fragments are innermost-first, so iterate them in REVERSE (outermost
    /// first) — the innermost then becomes the rightmost `unify` argument and
    /// wins on conflicting keys, matching `with`-scope shadowing.
    fn compile_fold_dict(
        ...
        // `{}` — empty record base. `knot_record_empty` takes a capacity hint.
        let (field, spans) = self.fold_dict_args.get(&span).cloned().unwrap_or_default();
        let cap = builder.ins().iconst(self.ptr_type, spans.len() as i64);
        let mut acc = self.call_rt(builder, "knot_record_empty", &[cap]);
        // Outermost-first: innermost fragment ends up rightmost in `unify`.
        for synth in spans.iter().rev() {
            let frag = self.compile_expr(
                builder,
                &ast::Expr {
                    node: ast::ExprKind::ImplicitRef(field.clone()),
                    span: *synth,
                },
                env,
                db,
            );
            // `knot_record_unify(a, b)` — right-biased field union.
            acc = self.call_rt(builder, "knot_record_unify", &[acc, frag]);
        }
        // Wrap the merged record as `{field : merged}` so the elaborated
        // body's `__dict_<field>.field` projection (see desugar) reads the
        // fold — matching the `(^field : T) =>` single-match dict shape.
        let one = builder.ins().iconst(self.ptr_type, 1);
        let wrapped = self.call_rt(builder, "knot_record_empty", &[one]);
        let (key_ptr, key_len) = self.string_ptr(builder, &field);
        self.call_rt_void(builder, "knot_record_set_field", &[wrapped, key_ptr, key_len, acc]);
        wrapped
    }
    ...
    fn compile_app(
        ...
        // A `(<>field : T) =>` FOLD constraint merges EVERY enclosing scope's
        // field via base.unify from `{}`; a `(^field : T) =>` single-match
        // splices the one resolved record.
        let mut compiled_args: Vec<Value> = Vec::new();
        if self.fold_dict_args.contains_key(&expr.span) {
            let dict_val = self.compile_fold_dict(builder, expr.span, env, db);
            compiled_args.push(dict_val);
    ...
    fn has_user_impls(&self, _method: &str) -> bool {
    ...
    fn sql_pushdown_disabled_by_user_impls(&self) -> bool {
        // No user impls can override operators anymore, so pushdown is safe.
        false
    }
