...
```
source.knot → Lexer → Tokens → Parser → AST → Type Inference → Effect Inference → Type Resolution → Cranelift IR → .o → cc link → executable
```
