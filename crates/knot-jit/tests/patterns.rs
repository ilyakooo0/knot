...
fn match_scrutinee_parenthesized_multiline_application() {
    // The `match` scrutinee must be parenthesized: `? (expr)`. Inside the
    // parens an application may span multiple lines (`? (add 3\n  4)`).
    assert_show(
        "with {\n_  add  (\\a b -> a + b)\n}\n(base.show (? (add 3\n  4)  _  \"done\"))",
        "done",
    );
    ...
    // An atom scrutinee is parenthesized too: `? (x)`.
    assert_show(
        "with {\n_  x  5\n}\n(base.show (? (x)\n  5  \"five\"\n  _  \"other\"))",
        "five",
    );
